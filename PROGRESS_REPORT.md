...
- AdamW is staged on shadow model/optimizer state. If applied-state persistence fails, coordinator gradients are restored and the unchanged `result_accepted` transaction can be retried; only successful publication installs the staged state. Evidence v2 binds the pre-update checkpoint and lets readers recompute the transaction ID from committed fields.
- Recovery took `3.00`–`3.05` seconds, dominated by `2.65`–`2.69` seconds of replacement initialization (`88.35%`–`88.42%`). Recovery retransmitted `3,684,408` bytes before replay and persisted `8,417,672` tensor bytes plus a `1,340`-byte manifest. Report 008 closes bounded P6 feasibility as qualified but conditional.
