...
The P4 follow-up fixed a 20-step T1 TinyStories trajectory and a profile-specific int8 control. Two-shard int8 aggregation stayed within `3.1242e-7` gradient relative L2 of centralized int8, final adapters stayed within `1.5513e-7`, and ten post-restart gradients/adapter/optimizer states were exact. Int8 held-out mean loss improved by `0.1061466932` versus `0.1049623489` FP32 while retaining 50.18% fewer model-tensor bytes. The int8 and FP32 adapters still diverged by 2.6366%, so int8 is promoted only to a homogeneous-profile candidate with a profile-bound oracle/checkpoint identity—not mixed exact aggregation. The current builder still constructs FP32 before quantizing; connected promotion requires a direct authenticated int8 loader and an explicit numerical-profile contract.
